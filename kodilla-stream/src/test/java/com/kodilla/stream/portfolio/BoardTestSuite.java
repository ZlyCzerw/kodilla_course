...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask(){
        ...
        //When
        List<TaskList> ongoingTasks = new ArrayList<>();

        ongoingTasks.add(new TaskList("To do"));
        ongoingTasks.add(new TaskList("In progress"));

        double longTasks = project.getTaskLists().stream()
                .filter(ongoingTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
        ...
        //Then
        Assert.assertEquals(14, longTasks, 0.0001);
    }
}
