package com.kodilla.patterns2a.adapter.company;

import com.kodilla.patterns2a.adapter.company.newhrsystem.CompanySalaryProcessor;
import com.kodilla.patterns2a.adapter.company.newhrsystem.Employee;
import com.kodilla.patterns2a.adapter.company.newhrsystem.SalaryProcessor;

import java.math.BigDecimal;
import java.util.List;

public class SalaryAdaptee implements SalaryProcessor {
    @Override
    public BigDecimal calculateSalaries(List<Employee> employees) {
        SalaryProcessor salaryProcessor = new CompanySalaryProcessor();
        return salaryProcessor.calculateSalaries(employees);
    }
}
